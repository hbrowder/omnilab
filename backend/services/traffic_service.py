...
from api.traffic_websocket import (
    send_error,
    send_filter_activated,
    send_filter_deactivated,
)
...
class TrafficService:
    ...
    async def start_capture(self, lab_id: str, filter_id: str, expression: str, color: str):
        ...
        with self._lock:
            ...
            try:
                ...
                for (container_name, interface), _link_id in container_interface_map.items():
                    try:
                        ...
                        for p in session.processes.values():
                            p.kill()
                        raise RuntimeError(f"Failed to start capture on {container_name}:{interface}: {e}") from e
    ...
    async def stop_capture(self, filter_id: str):
        ...
        with self._lock:
            ...
            for _iface, process in session.processes.items():
                try:
                    process.terminate()
